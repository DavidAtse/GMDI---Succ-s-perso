...
const TOKEN_KEY = 'gmdi_st_token';
const USER_KEY  = 'gmdi_st_user';
  ...
  login(creds: LoginRequest) {
    return this.http.post<LoginResponse>(`${environment.apiUrl}/auth/login`, creds).pipe(
      ...
      catchError(err => {
        ...
        if (err.status === 0)   msg = 'Serveur inaccessible (http://localhost:8000)';
        else if (err.status === 419) msg = 'Erreur CSRF (419) — relancez php artisan serve';
        else if (err.status === 422) msg = err.error?.errors?.email?.[0] ?? 'Identifiants incorrects';
        else if (err.status)    msg = `Erreur ${err.status} — ${err.error?.message ?? ''}`;
